function generate-completions --description "Generate and save completions for lazy-loading"
    ...
    # $__fish_config_dir is ~/.config/fish, which stow links into the dotfiles
    # checkout -- so this lands in the repo without hardcoding where it lives.
    # Check $HOMEBREW_PREFIX/share/fish/vendor_completions.d first: formulae
    # that ship their own completions (gh, docker, ...) need no snapshot here.
    set -l dest_dir "$__fish_config_dir/completions"
