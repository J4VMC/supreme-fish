function init-git --description "Initialize a git repository with Lefthook & Conventional Commits"
  ...
  commands:
    ...
    # 4. No `lefthook install`: ~/.gitconfig sets a global core.hooksPath, under
    #    which lefthook refuses to install (and --force would overwrite the global
    #    hooks). The global hooks already run `lefthook run <hook>` whenever a
    #    lefthook.yml is present, so the file above is live as soon as it exists.
